...
//        Assert.notNull(leafClass, "Class must not be null");
        ...
        Field[] all=new Field[searchTypeFields.length+superTypeFields.length];
        int count=0;
        for (int i = 0; i < searchTypeFields.length; i++) {
            all[i]=searchTypeFields[i];
            count++;
        }
        for (int i = 0; i < superTypeFields.length; i++) {
            all[count]=superTypeFields[i];
        }
        return all;
//        return (Field[]) ArrayUtils.appendToArray(searchTypeFields, superTypeFields);
    }
